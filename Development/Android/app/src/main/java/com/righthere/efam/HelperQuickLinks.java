...
public class HelperQuickLinks extends Activity {
	...
	public void create(Button menuIcon, Button resumeShopping, Button pay,
			TextView cartBtnNotification, Context ctx,
		 ...
		 *           if(sharedPreferences.contains("myTrolley")) { Intent intent
		 *           = new Intent(context, ActivityStep05ListAisles.class);
		 *           context.startActivity(intent); }else{
		 *           Toast.makeText(context, "No Shop Selected!",
		...
		pay.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (sharedPreferences.contains("myTrolley")) {
					...
					Toast.makeText(context, "No items in your cart!",
							Toast.LENGTH_LONG).show();
				}
