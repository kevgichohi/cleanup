...
public class AdapterProducts extends BaseAdapter implements
	...
	public AdapterProducts(Context ctx, ArrayList<RequestedResults> results,
			TextView cartBtnNotification, LinearLayout progressbar) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			convertView = mInflater
					.inflate(R.layout.simple_list_products, null);
			...
			// holder.removefromcart = (Button)
			// convertView.findViewById(R.id.removefromcart);
			// holder.itemtotalprice = (TextView)
			// convertView.findViewById(R.id.itemtotalprice);

			// holder.itemtotalprice.setVisibility(View.GONE);
		...
		if (android.text.TextUtils.isEmpty(price) == false
			...
			Double item_price_double = Double.parseDouble(searchArrayList.get(
					position).getPrice());
		...
		Integer total = searchArrayList.get(position).getUnits()
				* Integer.parseInt(searchArrayList.get(position).getPrice());
		...
		if (itemId.equals("0")) {
			...
			imageDownloader.download(searchArrayList.get(position)
					.getThumbnailUrl(), holder.thumbnail);
		}
	...
	public void onClick(View v) {
		...
		// ((TextView) view.findViewById(R.id.your_textview_id));
		Button addtocart = (Button) parentRow.findViewById(R.id.addtocart);
		// Button removefromcart =(Button)
		// parentRow.findViewById(R.id.removefromcart);
		...
		if (sharedPreferences.contains("myTrolley")) {
			...
			String jsonMyCartItemPriceString = sharedPreferences.getString(
					"myTrolleyItemPrice", (new JSONObject()).toString());
			try {
				Integer units = 1;
				JSONObject jsonMyCartObject = new JSONObject(jsonMyCartString);
				JSONObject jsonMyCartItemPriceObject = new JSONObject(
						jsonMyCartItemPriceString);
				if (jsonMyCartObject.has(item.item_id)) {
					...
					if (action.equals("add")) {// ADD UNITS
						...
						jsonMyCartItemPriceObject.put(item.item_id,
								item.item_title + "_" + item.item_price + "_"
										+ item.item_size);
					} else {// REMOVE UNITS
						units--;
						if (units.equals(0)) {
						...
						Toast.makeText(context, "Removed from cart",
								Toast.LENGTH_SHORT).show();
					}
				} else {
					// removefromcart.setVisibility(View.VISIBLE);
				...
				String newJsonMyCartItemPriceString = jsonMyCartItemPriceObject
						.toString();
				editor.putString("myTrolleyItemPrice",
						newJsonMyCartItemPriceString);
				editor.commit();
			...
			myCartItemPrice.put(item.item_id, item.item_title + "_"
					+ item.item_price + "_" + item.item_size);
			JSONObject jsonMyCartItemPriceObject = new JSONObject(
					myCartItemPrice);
			String jsonMyCartItemPriceString = jsonMyCartItemPriceObject
					.toString();
		...
		/**
		 * String unitsAdded = (String) addtocart.getText(); Integer newUnits =
		 * 1; if(!unitsAdded.isEmpty()){ String units =
		 * unitsAdded.replaceAll("[^0-9]", ""); if(action.equals("add")){ //ADD
		 * TO CART COUNT_CART_ITEMS ++; newUnits = Integer.parseInt(units) + 1;
		 * Toast.makeText(context, "Added to cart" , Toast.LENGTH_SHORT).show();
		 * }else{ //REMOVE FROM CART if(COUNT_CART_ITEMS > 0){ COUNT_CART_ITEMS
		 * --; } newUnits = Integer.parseInt(units) - 1; Toast.makeText(context,
		 * "Removed from cart" , Toast.LENGTH_SHORT).show(); }
		 * 
		 * }
		 * 
		 * if(newUnits.equals(0)){ addtocart.setText(""); }else{
