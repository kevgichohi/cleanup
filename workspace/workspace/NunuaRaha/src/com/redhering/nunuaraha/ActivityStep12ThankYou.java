...
public class ActivityStep12ThankYou extends Activity {
	...
	private static String selectedOutletId2;
	private static String reorderif2;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// DELIVERY OPTION
			...
			CUSTOMER_ADDRESS = sharedPreferences.getString("deliveryAddress",
					null);
		}

		TOTAL = sharedPreferences.getString("myTrolleyTotal", null);
		reciepttotaltextdata2 = sharedPreferences.getString(
		...
		CUSTOMER_ORDER_ITEM_PRICE = sharedPreferences.getString(
				"myTrolleyItemPrice", null);
		...
		if (sharedPreferences.contains("reorderif")) {
			...
			selectedOutletId2 = sharedPreferences
					.getString("newoutletid", null);
			...
			SELECTED_BRAND_ID = sharedPreferences.getString("selectedBrandId",
					null);
			selectedOutletId2 = sharedPreferences.getString("selectedOutletId",
					null);
		...
		Log.i("selectedOutletId2", selectedOutletId2);
		...
		new HttpAsyncTask()
				.execute("http://www.e-fam.com/mobile_trolley_app/saveorder.php");

		// LOAD QUICKLINKS
		HelperQuickLinks helperQuickLinks = new HelperQuickLinks();
		helperQuickLinks.create(menuIcon, shoppingButton, cartButton,
				cartButtonNotification, ActivityStep12ThankYou.this,
				sharedPreferences);

		shoppingEnded.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent(ActivityStep12ThankYou.this,
...

				{
					...
					public void onClick(DialogInterface dialog, int which) {
						overridePendingTransition(R.anim.slide_menu_in,
								R.anim.slide_menu_out);// SlideIn animation
	...
	private class HttpAsyncTask extends AsyncTask<String, Void, String> {
		...
		protected void onPostExecute(String result) {
			...
			// Toast.makeText(getBaseContext(), "Checkout",
			...
			try {
				...
				if (sharedPreferences.getString("deliverShopping", null)
						.equals("No")) {
				...
				// thankYouMessage.setText("Thank you " + CUSTOMER_FIRST_NAME +
				// " for shopping on E-fam at " + SELECTED_BRAND_TITLE + " " +
				// SELECTED_BRANCH_TITLE + " \nTotal: KES."+TOTAL);
				thankYouMessage.setText("Thank you " + CUSTOMER_FIRST_NAME
						+ " for shopping on E-fam");
				thankYouMessage2.setText("Your delivery will arrive in 1Hr at "
						+ CUSTOMER_ADDRESS);
				if (sharedPreferences.getString("deliverShopping", null)
						.equals("No")) {
					thankYouMessage2
							.setText("Please make sure to pick your shopping today before shop closes.Please note down the order number as you will need it to collect your shopping.Please carry along your identification.");
				}
				...
				sharedPreferences.edit().remove("reciepttotaltextdata2")
						.commit();
				sharedPreferences.edit().remove("deliverShopping").commit();
				sharedPreferences.edit().remove("reorder").commit();
	...
	public static String GET(String url) {
		...
		nameValuePairs.add(new BasicNameValuePair("customer_address",
				CUSTOMER_ADDRESS));
		nameValuePairs.add(new BasicNameValuePair("customer_order_item_price",
				CUSTOMER_ORDER_ITEM_PRICE));
		nameValuePairs.add(new BasicNameValuePair("transactionRef",
				Transaction_Ref));
		nameValuePairs.add(new BasicNameValuePair("customer_order",
				CUSTOMER_ORDER));
		nameValuePairs.add(new BasicNameValuePair("customer_order_total",
				reciepttotaltextdata2));
		nameValuePairs
				.add(new BasicNameValuePair("payment_pref", paymentOption));
		nameValuePairs.add(new BasicNameValuePair("branch_id",
				SELECTED_BRANCH_ID));
		nameValuePairs
				.add(new BasicNameValuePair("brand_id", SELECTED_BRAND_ID));
		nameValuePairs.add(new BasicNameValuePair("outlet_id",
				selectedOutletId2));

		// nameValuePairs.add(new
		// BasicNameValuePair("selectedOutletId",selectedOutletId));
